...
def get_refs(generators, parsets, iidmfile):
    ...
    for s in parsets:
        ...
        dyd_gen_id = [x['dyd'].attributes['id'].value for x in generators.values() if x['dyd'].attributes['parId'].value == s.attributes['id'].value][0]
        gen = [x for x in iidm_generators if x.attributes['id'].value in dyd_gen_id or dyd_gen_id in x.attributes['id'].value ]
        if len(gen) == 0:
        ...
        for r in refs:
            ...
            if 'P0Pu' in r_name:
                ...
                r.setAttribute('value', str(float(gen.attributes['targetV'].value)/float(gen.parentNode.attributes['nominalV'].value)))
            elif 'UPhase0' in r_name:
                bus = [x for x in iidm.getElementsByTagName('iidm:bus') if x.attributes['id'].value == gen.attributes['bus'].value][0]
